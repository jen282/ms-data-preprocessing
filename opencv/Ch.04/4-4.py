...
# 이미지 읽기 및 초기화
image = cv.imread('./opencv/Ch.04/soccer.jpg')
image_display = image.copy()
mask = np.full(image.shape[:2], cv.GC_PR_BGD, dtype=np.uint8)  # 기본은 불확실 배경
...
# 결과 적용
output = cv.bitwise_and(image, image, mask=result_mask)
